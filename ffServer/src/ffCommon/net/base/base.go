...
type Session interface {
	...
	// 发送 Proto 到对端, 异步，当Agent处理完毕MessageType_SessionDisConnect后，禁止再调用此方法
...
type Agent interface {
	...
	// protoID: MessageType_SessionDisConnect
	//				连接断开，Session以上层处理完毕该事件来作为Session结束的依据, data为sync.WaitGroup。
	//				处理完毕后，必须执行data.Done，以便Session逻辑继续执行。
	//				Agent执行完毕此事件后，即代表连接完全断开，禁止再调用Session的SendProto方法
	//			MessageType_SessionConnect
	//				连接建立，上层以该事件来作为Session有效的起点，data无效
